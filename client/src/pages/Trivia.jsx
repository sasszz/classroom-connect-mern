...
const Trivia = () => {
    ...
    const handleKeyDown = (event) => {
        ...
        setSelectedDivIndex((prevIndex) => (prevIndex - 1 + divsData.length) % divsData.length);
        }
    };
  ...
  return (
    <div className=''>
            ...
            <div>
                ...
                <div className='bg-black text-white'>
                    <ProgressBar
                        ...
                        className='text-left '
                    />
                    <div className='flex flex-wrap justify-center text-xl font-bold' onKeyDown={handleKeyDown} tabIndex="0">
                    ...
                    <div>
                        ...
                        {messages.map((message, index) => (
                            ...
                            >   
                                ...
                                <div className='italic text-sm text-black'>{new Date().toLocaleString([], { hour: 'numeric', minute: 'numeric', hour12: true })}
                                </div>
                            </span>
                            </div>
                        ))}
        ...
        <div className='bg-black text-white fixed bottom-0 left-0 right-0 flex flex-wrap text-lg p-3 justify-between'>
            ...
            <div style={{ cursor: 'pointer' }} onClick={() => toggleVisibility('leaderboard')} 
            className='border rounded p-2 m-2 flex justify-center items-center'>
                <MdLeaderboard className='m-1' />
                <div>LeaderBoard</div>
            </div>
            <div className='flex'>
                <div style={{ cursor: 'pointer' }} onClick={() => toggleVisibility('chat')} 
                className='border rounded p-2 m-2 flex justify-center items-center'>
                    <BsChatLeftText className='m-1'/>
                    <div>Chat</div>
                </div>
                <div style={{ cursor: 'pointer' }} onClick={() => toggleVisibility('participants')} 
                className='border rounded p-2 m-2 flex justify-center items-center'>
                    <BsPeople className='m-1'/>
                    <div>Students</div>
                </div>
